...
Напиши только письмо, без пояснений и комментариев.`;
    ...
    // Проверяем качество GPT письма
    if (line) {
      const isToShort = line.length < 200;
      const hasContent = ['смотрел', 'посмотрел', 'изучил', 'ваш', 'компани', 'продукт', 'услуг', 'каталог', 'производ'].some(w => line.toLowerCase().includes(w));
      const hasBadWords = ['эффективно', 'оптимизировать', 'инновацион', 'синергия', 'современн'].some(w => line.toLowerCase().includes(w));

      if (isToShort || !hasContent || hasBadWords) {
        console.log(`[GPT] fallback на шаблон для ${domain}: короткое=${isToShort} контент=${hasContent} плохиеслова=${hasBadWords}`);
        resultHtml = html; // возвращаем оригинальный шаблон
        subject = null;
      }
    }
...
async function sendEmail(item) {
  ...
  let html = item.html;
  const PUBLIC_DOMAINS = new Set(['mail.ru','yandex.ru','yandex.com','gmail.com','gmail.ru','inbox.ru','list.ru','bk.ru','rambler.ru','yahoo.com','outlook.com','hotmail.com','icloud.com','me.com','protonmail.com','vk.com']);
  const isPublicEmail = PUBLIC_DOMAINS.has((item.domain||'').toLowerCase());
  if (process.env.PROXYAPI_KEY && item.domain && item.personalize !== false && !isPublicEmail) {
