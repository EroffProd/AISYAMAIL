...
async function personalizeLine(domain, html) {
  ...
  try {
    ...
    const prompt = `Ты помогаешь писать персонализированные холодные письма от лица компании AISYA (автоматизация бизнес-процессов на базе ИИ).

На основе текста сайта компании напиши ТОЛЬКО персональный абзац (2-4 предложения) для вставки в начало письма.

Правила:
- Покажи что реально смотрел сайт: упомяни конкретный продукт, услугу или специфику бизнеса
- Назови конкретную операционную боль характерную для этого типа бизнеса (заявки, номенклатура, счета, карточки клиентов, согласования и т.д.)
- Пиши живо, как человек — не как робот
- ЗАПРЕЩЕНО: слова "эффективно", "оптимизировать", "значительно", "инновационный", "современный"
- ЗАПРЕЩЕНО: длинное тире — (это маркер нейросети), используй только запятые и точки
- ЗАПРЕЩЕНО: вводные фразы типа "Я заметил что", "Изучив ваш сайт"
- Короткие предложения
- Без приветствия, без подписи, только абзац

Примеры хорошей персонализации:

Пример 1 (магазин экипировки):
"Смотрел ваш каталог, у вас типичная для экипировочных сетей сложная номенклатура: от берцев и софтшеллов до противоосколочных комплектов. В таких магазинах менеджеры обычно тратят много времени на одно и то же: подбор позиций, уточнения по наличию, сбор комплектации, счета, занесение клиента в 1С."

Пример 2 (оптовая торговля):
"Посмотрел ваш сайт, у вас очень показательная для оптовиков модель: большой каталог и параллельно работа с корпоративными клиентами. В таких компаниях менеджеры часто половину времени тратят не на продажи, а на поток операционных вещей: заявки, счета, карточки клиентов в 1С."

Текст сайта компании: ${siteText}`;

    // Параллельно генерируем абзац и тему
    const [line, subject] = await Promise.all([
      gptRequest(prompt, 200),
    ...
    let resultHtml = html;
    if (line) {
      const personalBlock = `<p style="color:#1a1a1a;margin:0 0 14px">${line}</p>`;
      resultHtml = html.replace(/(<p[^>]*>)/i, `${personalBlock}$1`);
    }

    return { html: resultHtml, subject: subject || null };
...
async function sendEmail(item) {
  ...
  // Персонализация если включена
  let html = item.html;
  if (process.env.PROXYAPI_KEY && item.domain) {
    const result = await personalizeLine(item.domain, html);
    html = result.html;
    if (result.subject) item.subject = result.subject; // заменяем тему
  }
...
app.post('/send-now', async (req,res) => {
  ...
  try {
    ...
    if (r.ok && data.id) {
      res.json({ ok:true, id:data.id, from:sender.from });
    } else {
