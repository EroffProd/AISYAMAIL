...
app.get('/2gis', async (req, res) => {
  ...
  try {
    // Поиск с полными контактами
    const searchUrl = `https://catalog.api.2gis.com/3.0/items?q=${encodeURIComponent(q)}&fields=items.name,items.contact_groups,items.external_content,items.org&page_size=${page_size||50}&key=${apiKey}&locale=ru_RU&type=branch`;
    console.log('[2GIS] url:', searchUrl);
    const searchResp = await fetch(searchUrl, { headers: { 'Accept': 'application/json', 'User-Agent': 'Mozilla/5.0' } });
    const searchData = await searchResp.json();
    const firstItem = searchData?.result?.items?.[0];
    console.log('[2GIS] items:', searchData?.result?.items?.length || 0, 'first:', firstItem ? JSON.stringify(firstItem).slice(0,300) : 'none');
    res.json(searchData);
  } catch(e) {
    console.log('[2GIS] error:', e.message);
