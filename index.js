...
// Прокси для 2GIS API — поиск + детали каждой компании
app.get('/2gis', async (req, res) => {
  ...
  try {
    ...
    const searchUrl = `https://catalog.api.2gis.com/3.0/items?q=${encodeURIComponent(q)}&fields=items.name,items.id&page_size=${page_size||50}&key=${apiKey}&locale=ru_RU&type=branch`;
    const searchResp = await fetch(searchUrl, { headers: { 'Accept': 'application/json' } });
    const searchData = await searchResp.json();
    const items = searchData?.result?.items || [];

    // Шаг 2: получаем детали (contact_groups) батчами по 50
    const ids = items.map(i => i.id).join(',');
    if (!ids) { res.json({ result: { items: [] } }); return; }

    const detailUrl = `https://catalog.api.2gis.com/3.0/items/byid?id=${ids}&fields=items.name,items.contact_groups&key=${apiKey}&locale=ru_RU`;
