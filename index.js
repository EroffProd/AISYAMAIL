...
// Используем Resend API (HTTP) — Railway не блокирует
// ══════════════════════════════════════
const SENDERS = [
  { id: 'erofeev', from: 'Данил Ерофеев <erofeev@aisya.online>', dailyLimit: 80 },
  { id: 'aiprod',  from: 'AISYA <ai-prod@aisya.online>',         dailyLimit: 80 },
  { id: 'ceo',     from: 'Данил Ерофеев <ceo@aisya.ru>',         dailyLimit: 80 },
];
...
// ══════════════════════════════════════
function pickSender() {
  const available = SENDERS.filter(s => (sentToday[s.id] || 0) < s.dailyLimit);
...
async function sendEmail(item) {
  ...
  if (!sender) {
    item.status = 'error';
    item.error = 'Нет доступных отправителей (лимит)';
    return;
  }
  ...
  if (!RESEND_KEY) {
    item.status = 'error';
    item.error = 'RESEND_API_KEY не задан в переменных Railway';
    return;
  }
  ...
  try {
    ...
    if (res.ok && data.id) {
      ...
      console.log(`✓ Sent to ${item.to} via ${sender.from} | resend id: ${data.id}`);
    } else {
      item.status = 'error';
    ...
    console.error(`✗ Error sending to ${item.to}:`, err.message);
  }
}
...
app.get('/status', (req, res) => {
  ...
  res.json({
    ...
    accounts: SENDERS.map(s => ({ id: s.id, from: s.from, sentToday: sentToday[s.id] || 0, limit: s.dailyLimit })),
    queue: queue.slice(-50).map(i => ({ id: i.id, to: i.to, status: i.status, sentFrom: i.sentFrom, addedAt: i.addedAt, sentAt: i.sentAt, error: i.error })),
  });
});
