...
async function getBrowser() {
  if (!browserInstance) {
    ...
    let chromePath = '';
    // Пробуем все возможные пути включая nix store
    const candidates = [
    ...
    if (!chromePath) {
      try { chromePath = execSync('find /nix -name "chromium" -type f 2>/dev/null | head -1').toString().trim(); } catch(e) {}
    }
    console.log('[2GIS] chromePath:', chromePath);
...
app.get('/2gis', async (req, res) => {
  ...
  try {
    ...
    console.log('[2GIS] title:', pageTitle, 'url:', pageUrl);
    console.log('[2GIS] body:', bodyText);

    // Ждём карточки компаний
    await page.waitForSelector('[data-testid="search-results"] ._1ot7u', { timeout: 10000 }).catch(() => {});
    ...
    // Логируем что есть на странице
    const allSelectors = await page.evaluate(() => {
      const s = new Set();
      document.querySelectorAll('[class]').forEach(el => el.className.split(' ').forEach(c => s.add(c)));
      return Array.from(s).slice(0, 30).join(', ');
    });
    console.log('[2GIS] selectors:', allSelectors);

    // Собираем данные
    const items = await page.evaluate((limit) => {
      const results = [];
      const cards = document.querySelectorAll('._1ot7u, [data-testid="search-item"]');
      for (const card of Array.from(cards).slice(0, limit)) {
        const name = card.querySelector('._2lnqs, .bzjZ9, ._1Oy5h')?.textContent?.trim();
        if (!name) continue;
        ...
        for (const a of links) {
          const href = a.href || '';
          if (href.startsWith('http') && !href.includes('2gis.ru') && !href.includes('yandex') && href.includes('.')) {
            website = href.replace(/^https?:\/\//, '').replace(/\/$/, '').toLowerCase().replace(/^www\./, '');
            break;
          }
        }

        const phoneEl = card.querySelector('[class*="phone"], [href^="tel:"]');
        const phone = phoneEl?.textContent?.trim() || phoneEl?.href?.replace('tel:', '') || '';
    ...
    await page.close();
    console.log('[2GIS] найдено:', items.length, 'с сайтами:', items.filter(i=>i.website).length);

    // Возвращаем в том же формате что ожидает base.js
    res.json({
      result: {
        items: items.map(i => ({
          name: i.name,
          contact_groups: [{
            contacts: [
              ...(i.website ? [{ type: 'website', value: 'https://' + i.website }] : []),
              ...(i.phone   ? [{ type: 'phone',   value: i.phone }] : [])
            ]
          }]
        }))
      }
    ...
    console.log('[2GIS] ошибка:', e.message);
    // Сбросить браузер при ошибке
